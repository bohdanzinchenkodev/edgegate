...
printf 'GET http://%s:%s/svc0/get\n' "$PROXY_HOST" "$PROXY_PORT" > "$TARGETS_FILE"

echo "Running proxy load test"
echo "target=http://$PROXY_HOST:$PROXY_PORT/svc0/get rate=$RATE duration=$DURATION workers=$WORKERS connections=$CONNECTIONS"
