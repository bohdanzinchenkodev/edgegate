...
// ApplyConfigFunc is the function the controller calls to apply new config.
// In production, this will be egproxy.ApplyConfig.
...
func NewGatewayReconciler(c client.Client, gatewayName client.ObjectKey, applyFn ApplyConfigFunc) *GatewayReconciler {
...
func (gr *GatewayReconciler) Reconcile(ctx context.Context, req reconcile.Request) (reconcile.Result, error) {
	var gw gwv1.Gateway
	if err := gr.client.Get(ctx, gr.gatewayName, &gw); err != nil {
		return reconcile.Result{}, client.IgnoreNotFound(err)
	}

	if gw.Spec.GatewayClassName != controllerName {
		return reconcile.Result{}, nil
	}

	// List all HTTPRoutes and filter for ones referencing this Gateway.
...
func (gr *GatewayReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&gwv1.Gateway{}).
		Watches(&gwv1.HTTPRoute{}, handler.EnqueueRequestForOwner(mgr.GetScheme(), mgr.GetRESTMapper(), &gwv1.Gateway{})).
		Complete(gr)
}
